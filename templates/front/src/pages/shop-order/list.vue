                        ...
                        if(item.type==1||item.type==3) {
                            ...
                            this.$http.post(table+'/update', this.user).then(res => {
                                ...
                                this.$http.post(`orders/update`, item).then(res => {
                                    if(item.tablename == `ertongtushu`){
                                        // 加回库存数量
                                        good.alllimittimes = good.alllimittimes + item.buynumber;
                                        this.$http.post(`ertongtushu/update`, good).then(res => {
                                        });
                                    }
                                    if(item.tablename == `rexiaotushu`){
                                        // 加回库存数量
                                        good.alllimittimes = good.alllimittimes + item.buynumber;
                                        this.$http.post(`rexiaotushu/update`, good).then(res => {
                                        });
                                    }
                                    this.$message({
